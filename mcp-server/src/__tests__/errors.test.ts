...
describe('MCP Error Handling Utilities', () => {
  describe('formatApiError', () => {
    ...
    // Edge case tests for missing HTTP status codes
    it('should handle HTTP 403 Forbidden', () => {
      const error = { status: 403, message: 'Access forbidden' };
      const result = formatApiError(error);
    ...
    it('should handle HTTP 502 Bad Gateway', () => {
    ...
    it('should handle HTTP 504 Gateway Timeout', () => {
    ...
    it('should handle HTTP 409 Conflict', () => {
      const error = { status: 409, message: 'Resource conflict' };
      const result = formatApiError(error);
    ...
    // Edge case tests for invalid status codes
    it('should handle NaN status code', () => {
      const error = { status: NaN, message: 'Invalid status' };
      const result = formatApiError(error);

      expect(result.isError).toBe(true);
      expect(result.code).toBe('INVALID_STATUS');
      expect(result.message).toContain('Invalid status code');
    });

    it('should handle negative status code', () => {
      const error = { status: -1, message: 'Negative status' };
      const result = formatApiError(error);
    ...
    it('should handle Infinity status code', () => {
      const error = { status: Infinity, message: 'Infinite status' };
      const result = formatApiError(error);
    ...
    it('should handle float status code', () => {
      const error = { status: 200.5, message: 'Float status' };
      const result = formatApiError(error);
    ...
    // Edge case tests for invalid message types
    it('should handle null message', () => {
      const error = { status: 400, message: null as any };
      const result = formatApiError(error);

      expect(result.isError).toBe(true);
      expect(result.message).toBe('Invalid error message format');
    });

    it('should handle undefined message', () => {
      const error = { status: 400, message: undefined as any };
      const result = formatApiError(error);
    ...
    it('should handle object message', () => {
      const error = { status: 400, message: { error: 'nested' } as any };
      const result = formatApiError(error);
  ...
  describe('withErrorBoundary', () => {
    ...
    // Edge case tests for type guard (isApiError) weaknesses
    it('should reject API errors with non-string message', async () => {
      const error = { status: 400, message: 123 as any };
      ...
      expect(result.isError).toBe(true);
      // Should NOT be treated as valid API error due to invalid message type
      expect(result.code).toBe('UNKNOWN_ERROR');
    });

    it('should reject API errors with prototype pollution attack', async () => {
      const maliciousError = Object.create({ status: 400, message: 'inherited' });
      const handler = vi.fn().mockRejectedValue(maliciousError);
      const wrapped = withErrorBoundary(handler);

      const result = await wrapped({}) as McpErrorResponse;

      expect(result.isError).toBe(true);
      // Should NOT be treated as valid API error when properties are inherited
      expect(result.code).toBe('UNKNOWN_ERROR');
    });

    it('should reject API errors with invalid status codes via type guard', async () => {
      const error = { status: NaN, message: 'test' };
      const handler = vi.fn().mockRejectedValue(error);
      const wrapped = withErrorBoundary(handler);
      ...
      expect(result.isError).toBe(true);
      // Should NOT pass type guard with NaN status
      expect(result.code).toBe('UNKNOWN_ERROR');
    });
  });

  describe('McpErrorResponse structure', () => {
    it('should always include isError, code, and message fields', () => {
      const error = formatApiError({ status: 500, message: 'Test' });
