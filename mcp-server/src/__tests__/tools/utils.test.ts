...
describe('Utility Tools', () => {
  ...
  describe('log (simple shorthand)', () => {
    describe('happy path', () => {
      ...
      it('should support multi-word messages', async () => {
        const mockResponse = {
          data: {
            success: true,
            logged: {
              timestamp: '2024-01-15T10:30:00Z',
              agent: 'Amy',
              message: 'Found bug in auth flow - user can bypass login',
            },
          },
          status: 200,
          headers: {},
        ...
        const parsed = JSON.parse(result.content[0].text);
        expect(parsed.logged.message).toBe('Found bug in auth flow - user can bypass login');
      });
    });
  ...
  describe('Tool Registration', () => {
    it('should export tool definitions for MCP server registration', async () => {
      const { utilsTools } = await import('../../tools/utils.js');

      expect(utilsTools).toBeDefined();
      expect(Array.isArray(utilsTools)).toBe(true);
      expect(utilsTools.length).toBe(3);

      const toolNames = utilsTools.map((t: any) => t.name);
      expect(toolNames).toContain('deps_check');
      expect(toolNames).toContain('activity_log');
  ...
  describe('Response Structure Consistency', () => {
    it('deps_check should return content array with text type', async () => {
      mockClient.get.mockResolvedValueOnce({
        data: { valid: true, cycles: [], depths: {}, maxDepth: 0, parallelWaves: 0, readyItems: [], totalItems: 0 },
        status: 200,
      });
    ...
    it('activity_log should return content array with text type on success', async () => {
      mockClient.post.mockResolvedValueOnce({
        data: { success: true, logged: { timestamp: '2024-01-15T10:00:00Z', agent: 'Murdock', message: 'test' } },
        status: 200,
      });

      const { activityLog } = await import('../../tools/utils.js');

      const result = await activityLog({ agent: 'murdock', message: 'test' });

      expect(result.content).toBeInstanceOf(Array);
      expect(result.content[0]).toHaveProperty('type', 'text');
    });

    it('log should return content array with text type on success', async () => {
      mockClient.post.mockResolvedValueOnce({
        data: { success: true, logged: { timestamp: '2024-01-15T10:00:00Z', agent: 'Murdock', message: 'test' } },
        status: 200,
      });
      ...
      const result = await log({ agent: 'murdock', message: 'test' });
