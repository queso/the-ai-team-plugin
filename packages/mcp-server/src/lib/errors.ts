...
/**
 * Network error with optional code property.
 */
 ...
 */
function isValidHttpStatusCode(status: number): boolean {
  return (
 ...
 */
export function formatApiError(error: ApiError): McpErrorResponse {
 ...
 */
export function formatNetworkError(error: Error): McpErrorResponse {
...
): (args: TArgs) => Promise<TResult | McpErrorResponse> {
  return async (args: TArgs): Promise<TResult | McpErrorResponse> => {
    try {
